...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
                    ...
                    <table align="center" border="0" cellpadding="0" cellspacing="0" class="container" width="100%" style="max-width: 600px; background-color: #11121a; margin: 20px auto; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.5);">
                        ...
                        <tr>
                            <td style="padding: 30px 30px;">
                                ...
                                <p style="font-size: 16px; line-height: 1.6; margin-bottom: 20px; color: #e6e6ef;">Thank you for Signin in! We are excited to have you back on our platform</p>
                                
                                <p style="font-size: 16px; line-height: 1.6; margin-bottom: 25px; color: #e6e6ef;">To complete your login process, please use the One-Time Password (OTP) below:</p>
        ...
        $deleteStmt = $conn->prepare("DELETE FROM verification_codes WHERE email = ? AND purpose = 'login'");
        $deleteStmt->bind_param("s", $email);
        $deleteStmt->execute();
        $deleteStmt->close();

        $stmt = $conn->prepare("INSERT INTO verification_codes (email, code, created_at, expires_at, purpose) VALUES (?, ?, NOW(), ?, 'login')");
        ...
        $message = "Login OTP sent to: - $email: - " . $otp;
