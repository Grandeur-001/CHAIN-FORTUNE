...
<body>
    ...
    <main class="main" id="main">
        <div class="app-container">
            <div class="header">
                <h1>Withdrawal Transaction Records</h1>
            ...
            <div class="transaction-table-wrapper">
                <table class="transaction-table">
                   <div>
                        ...
                        <?php include("../../../backend/fetch_withdrawals.php") ?>
                        <tbody>
                            <?php foreach ($transactions as $transaction) : ?>
                                <tr class="transactions-list" id="transactions-row-<?= htmlspecialchars($transaction['user_id']) ?>">
                                    ...
                                    <td data-label="Transaction ID">
                                        <a href="/chain-fortune/admin/transaction_details?hash=<?= htmlspecialchars($transaction['transaction_id']) ?>" class="hash-link" style="width: 150px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?= htmlspecialchars($transaction['transaction_id']) ?></a>
                                    </td>
                                    ...
                                    <td data-label="Actions">
                                        <div class="btn-group">
                                            ...
                                            <div class="action-dropdown-menu">
                                                <button class="dropdown-item update_transaction" name="update" onclick="openUpdateTransaction('update-transaction-modal-<?= htmlspecialchars($transaction['transaction_id']) ?>')">Update</button>
                                                <button class="dropdown-item view_transaction" name="view" onclick="location.href='transaction_details.php?hash=<?= htmlspecialchars($transaction['transaction_id']) ?>'">View</button>
                                                <button class="dropdown-item delete_transaction" name="delete" onclick="openDeleteTransaction('delete_transaction_<?= htmlspecialchars($transaction['transaction_id']) ?>')">Delete</button>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <div class="action-modal overlay update-transaction-modal" id="update-transaction-modal-<?= htmlspecialchars($transaction['transaction_id']) ?>" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
                                    ...
                                    <div class="action-modal-content">
                                        <div class="action-modal-header">
                                            <h5 class="action-modal-title" id="modalTitle">Update Transaction</h5>
                                            <button class="action-modal-close close_action" aria-label="Close">&times;</button>
                                        </div>
                                        <div class="action-modal-body">
                                        <p>You can confirm or decline this transaction</p>
                                        </div>
                                        <div class="action-modal-footer">
                                            ...
                                            <button 
                                                ...
                                                class="btn btn-primary decline-withdrawal-btn" 
                                                name="decline-withdrawal-btn" 
                                                id="decline-withdrawal-btn" 
                                                data-transaction-id="<?= htmlspecialchars($user['transaction_id']) ?>">
                                                Decline
                                            </button>
                                            
                                            <button 
                                                style="background: var(--accent-clr)" 
                                                class="btn btn-primary confirm-withdrawal-btn" 
                                                name="confirm-withdrawal-btn" 
                                                id="confirm-withdrawal-btn" 
                                                data-transaction-id="<?= htmlspecialchars($transaction['transaction_id']) ?>">
                                                Confirm
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <div class="action-modal overlay delete_transaction_section" id="delete_transaction_<?= htmlspecialchars($transaction['transaction_id']) ?>" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
                                    <div class="action-modal-content">
                                        <div class="action-modal-header">
                                            <h5 class="action-modal-title" id="modalTitle">Update Transaction</h5>
                                            <button class="action-modal-close close_action" aria-label="Close">&times;</button>
                                        ...
                                        <div class="action-modal-body">
                                            ...
                                            <div style="display: flex; align-items: center;">
                                                <samp style="margin-right: 5px; color: var(--negative-text-clr)">This cannot be undone</samp>
                                                <img src="/chain-fortune/images/svg/warning-alt-svgrepo-com.svg" alt=""> 
                                            </div>
                                        </div>
                                        <div class="action-modal-footer">
                                            <button class="btn btn-secondary close_action" id="closeModal">Close</button>
                                            ...
                                            <button 
                                                ...
                                                style="background: var(--negative-text-clr)" 
                                                class="btn btn-primary delete-transaction-btn" 
                                                name="delete_transaction" 
                                                id="delete_transaction" 
                                                data-user-id="<?= htmlspecialchars($transaction['user_id']) ?>">
    ...
    <script>    
        ...
        function openDeleteTransaction(sectionId) {
            const $section = $(`#${sectionId}`);
            if ($section.length === 0) return;

            $('.delete_transaction_section').css({
                visibility: 'hidden',
                opacity: '0'
            });
    ...
    <script>
        $('.confirm-withdrawal-btn').off('click').on('click', function() {
            const userId = $(this).closest('.action-modal').find('input[name="user_id"]').val();
            const transactionId = $(this).closest('.action-modal').find('input[name="transaction_id"]').val();
            const amount = $(this).closest('.action-modal').find('input[name="amount"]').val();
            const cryptoSymbol = $(this).closest('.action-modal').find('input[name="crypto_symbol"]').val();
            const status = $(this).closest('.action-modal').find('input[name="status"]').val();
        ...
        $('.decline-withdrawal-btn').off('click').on('click', function() {
            const modal = $(this).closest('.action-modal');
            const userId = modal.find('input[name="user_id"]').val();
            const transactionId = modal.find('input[name="transaction_id"]').val();
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'This will decline the transaction.',
                icon: 'warning',
